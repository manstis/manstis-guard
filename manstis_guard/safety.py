...
class ManstisGuardImpl(Safety, ShieldsProtocolPrivate):

    def __init__(self, config: ManstisGuardConfig, deps) -> None:
        self.config = config
    ...
    async def run_shield(
        ...
        # [TODO] Ensure this is the latest User message
        message: UserMessage = messages[len(messages)-1:][0]
        model_id = self.config.model_id

        impl = ManstisGuardShield(
            model_id=model_id,
...
class ManstisGuardShield:
    ...
    def build_text_shield_input(message: UserMessage) -> UserMessage:
        return UserMessage(content=ManstisGuardShield.build_prompt(message))
    ...
    async def run(self, message: UserMessage) -> RunShieldResponse:
        shield_input_message = ManstisGuardShield.build_text_shield_input(message)
        ...
        return ManstisGuardShield.get_shield_response(content)
